...
// 문제 풀이
// 아메리카노 4500원 / 라떼 5000원
// anything === iceamericano | americanoice
// ice와 hot을 구분해야함
function solution(order) {
  ...
  order.map((beverage, idx) => {
    if (beverage.includes("americano")) {
      americano++;
    } else if (beverage.includes("anything")) {
      americano++;
    } else {
      cafelatte++;
    }
  });
  totalCost = americano * 4500 + cafelatte * 5000;

  return totalCost;
}

solution(["cafelatte", "americanoice", "hotcafelatte", "anything"]); // 19000
// solution(["americanoice", "americano", "iceamericano"]); // 13500
